...
/*
 * Still fixed values ...
 * The SCLK value must never exceed PCLK/2
 */
#ifdef USE_XTAL_OSC
...
#define	CS_LOW()	{LPC_GPIO0->FIOCLR = 1<<6;}	/* Set P0.6 low */
#define	CS_HIGH()	{LPC_GPIO0->FIOSET = 1<<6;}	/* Set P0.6 high */
#define PCSSP		(1<<10)
#define	PCLKSSP		PCLK_SSP1
#endif
...
{
	LPC_SC->PCONP |= PCSSP;	    	/* Enable SSP module */
#if SSP_CH == 0
	SC->PCLKSEL1 &= ~(3<<10);	/* Select PCLK frequency for SSP */
	SC->PCLKSEL1 |= (1<<10);
	...
	/*
	 * Set P0.6 = GPIO output, P0.7 = SCK1, P0.8 = MISO1, P0.9 = MOSI1
	 */
	LPC_PINCON->PINSEL0 &= ~(0b11111111 << 12);
	LPC_PINCON->PINSEL0 |=   0b10101000;
