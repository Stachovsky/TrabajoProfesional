...
/*
 * RWJ: Added this to enable snprintf() to call snprintf()
 */
extern int nsprintf(char *buffer, int buffer_length, const char *format, ...)
    __attribute__ ((format (printf, 3, 4)));
#undef snprintf
#define snprintf(...) 		( nsprintf(__VA_ARGS__))
