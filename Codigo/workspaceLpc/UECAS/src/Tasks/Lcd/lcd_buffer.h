...
// Filler the buffer
extern void buf_fill(void);

void buf_clean_rectangle(unsigned char xi, unsigned char yi, unsigned char xf, unsigned char yf);

// Set the position where text & shapes will be drawn
#define buf_set_position(x, y) (buf_x = (x), buf_y = (y))
//extern unsigned char buf_x, buf_y;

// Set the font
#define buf_set_font(f) (buf_font = (f))
//extern unsigned char * data buf_font;
...
extern void buf_draw(unsigned char *shape);
...
// to it if you like!
//extern unsigned char lcd_buffer[1024];

unsigned char buf_x, buf_y;
unsigned char * buf_font;
unsigned char lcd_buffer[1024];
unsigned char BuffGetCharDim ( unsigned char );
